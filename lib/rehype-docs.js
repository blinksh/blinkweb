...
export default function rehypeDocs({ filePath, tag }) {
  ...
  const blobUrl = `${GITHUB_URL}/${REPO_NAME}/blob/raw`;
  ...
  function visitAnchor(node) {
    ...
    let isDocs = href.startsWith('/docs');
    const isAbsoluteUrl = ABSOLUTE_URL.test(href);

    if (!isAbsoluteUrl && href.endsWith('.md')) {
      isDocs = true;
    }
  ...
  function visitImage(node) {
    const props = node.properties;

    const isAbsoluteUrl = ABSOLUTE_URL.test(props.src);
    if (isAbsoluteUrl) {
      return;
    }

    let resolvedPath = resolve(filePath, props.src);
    props.src = getRawFileUrl(resolvedPath, tag || 'raw');
