...
import { getCurrentTag, fetchDocsManifest } from "../lib/docs";
...
async function indexDocs() {
  const client = algoliasearch("KV8PHW34N4", process.env.ALGOLIA_API_KEY);
  // Init the docs index, this will throw if the index doesn't exist
  const index = await client.initIndex("blink-docs");
  ...
  const recordsByFile = await Promise.all(
    files.map((filePath) => addRecords(filePath, tag))
  );
